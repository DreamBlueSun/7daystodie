...
public class ArmorPartDescT2 {

    private static final Map<String, ArmorInfo> SKILL_INFO;

    static {
        try {
            String armorT2Skill = Files.readString(Paths.get("D:\\workspace-idea\\7daystodie\\iridescent\\src\\main\\java\\ume\\marisa\\iridescent\\items\\armor\\ArmorT2Skill.txt"));
            SKILL_INFO = new HashMap<>();
            for (String skill : armorT2Skill.split("\r\n")) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (KV<String, Constants.Zombie> zombie : Constants.ZOMBIES) {
            ...
            String replace = armor
                    ...
                    .replace("T1", "T2")
                    .replace("基础", "晶化")
                    .replace("阿琳", "晶化" + zombie.getV().getLocation())
                    .replace("RANK 1", zombie.getV().getRankT2())
                    .replace("999498", Constants.RANK_COLOR_MAP.get(zombie.getV().getRankT2().replace("RANK ", "")).replace("#", ""))
